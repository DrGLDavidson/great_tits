...
eyes_106_2020<-read.delim(file= "C1256RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_106_2020)


#🧽 CLEANING DATA ---- 

eyes_106_2020_f<-eyes_106_2020%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_106_2020_f) # flituring the data


names(eyes_106_2020_f)[names(eyes_106_2020_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_106_2020_f$RFID)
uniqueIDs #lets you see the different ids in the df
#"" "TagID_1 "   "0300030EFF" "011016FA48""0110170C32" "0110170740"


eyes_106_2020_f<-eyes_106_2020_f[!grepl('TagID_1', eyes_106_2020_f$RFID),]# removing tagid one 

eyes_106_2020_f<-eyes_106_2020_f[!eyes_106_2020_f$RFID=="",]#remove blank rows
head(eyes_106_2020_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_106_2020_f$RFID)
uniqueID #only 3 different individuals
#"0300030EFF" "011016FA48" "0110170C32" "0110170740"

#"0300024FEF", "0300030EFF" -> is the start and stop indicator 

referencetag<-eyes_106_2020_f[eyes_106_2020_f$RFID == '0300030EFF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used
# different time written down online 


eyes_106_2020_f<-eyes_106_2020_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_106_2020_f$Date) # changing the data format to POSIXct class
...
eyes_106_2020_t<-subset(eyes_106_2020_f,Date >= as.POSIXct('2020-05-20 08:44:05', tz="UTC")) # do from last indicator

#this time is 45 minutes from the above time manually write this
eyes_106_2020_t2<-subset(eyes_106_2020_t,Date <= as.POSIXct('2020-05-20 09:31:41', tz="UTC")) # make sure 45 mins
#not exactly 45mins 
...
eyes_106_2020_b.3<-eyes_106_2020_t2

eyes_106_2020_b.3<-cbind(eyes_106_2020_b.3, nestbox='mw106') 
eyes_106_2020_b.3<-cbind(eyes_106_2020_b.3, year='2020')
head(eyes_106_2020_b.3)
...
latency_eyes_106_2020<-eyes_106_2020_b.3%>%
...
latency_final_eyes_106_2020<-eyes_106_2020_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_106_2020)
glimpse(latency_final_eyes_106_2020)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_106_2020,file="totalVisits45m_mw106_2020.csv")
