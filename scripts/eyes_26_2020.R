...
eyes_26_2020<-read.delim(file= "C1254RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_26_2020)


#🧽 CLEANING DATA ---- 

eyes_26_2020_f<-eyes_26_2020%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_26_2020_f) # flituring the data


names(eyes_26_2020_f)[names(eyes_26_2020_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_26_2020_f$RFID)
uniqueIDs #lets you see the different ids in the df
# "" "TagID_1 "   "0300030EFF" "01103F5E08" "01103F5952" "0110170A4E"


eyes_26_2020_f<-eyes_26_2020_f[!grepl('TagID_1', eyes_26_2020_f$RFID),]# removing tagid one 

eyes_26_2020_f<-eyes_26_2020_f[!eyes_26_2020_f$RFID=="",]#remove blank rows
head(eyes_26_2020_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_26_2020_f$RFID)
...
referencetag<-eyes_26_2020_f[eyes_26_2020_f$RFID == '0300030EFF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used
# different time written down online 


eyes_26_2020_f<-eyes_26_2020_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_26_2020_f$Date) # changing the data format to POSIXct class
...
eyes_26_2020_t<-subset(eyes_26_2020_f,Date >= as.POSIXct('2020-05-19 09:39:15', tz="UTC")) # do from last indicator

#this time is 45 minutes from the above time manually write this
eyes_26_2020_t2<-subset(eyes_26_2020_t,Date <= as.POSIXct('2020-05-19 10:22:46', tz="UTC")) # make sure 45 mins
# missing the final date 

# 📅 DATE ----
# adding the date and the nest box name to the df

eyes_26_2020_b.3<-eyes_26_2020_t2

eyes_26_2020_b.3<-cbind(eyes_26_2020_b.3, nestbox='mw26') 
eyes_26_2020_b.3<-cbind(eyes_26_2020_b.3, year='2020')
head(eyes_26_2020_b.3)
...
latency_eyes_26_2020<-eyes_26_2020_b.3%>%
...
latency_final_eyes_26_2020<-eyes_26_2020_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_26_2020)
glimpse(latency_final_eyes_26_2020)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_26_2020,file="totalVisits45m_mw26_2020.csv")
