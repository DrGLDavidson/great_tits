...
eyes_15_2019<-read.delim(file= "C1252RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_15_2019)


#🧽 CLEANING DATA ---- 

eyes_15_2019_f<-eyes_15_2019%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_15_2019_f) # flituring the data


names(eyes_15_2019_f)[names(eyes_15_2019_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_15_2019_f$RFID)
uniqueIDs #lets you see the different ids in the df
# ""           "TagID_1 "   "0300024FEF" "011016FA48" "0110170AED" "011016FB39"

eyes_15_2019_f<-eyes_15_2019_f[!grepl('TagID_1', eyes_15_2019_f$RFID),]# removing tagid one 

eyes_15_2019_f<-eyes_15_2019_f[!eyes_15_2019_f$RFID=="",]#remove blank rows
head(eyes_15_2019_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_15_2019_f$RFID)
...
referencetag<-eyes_15_2019_f[eyes_15_2019_f$RFID == '0300024FEF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used


eyes_15_2019_f<-eyes_15_2019_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_15_2019_f$Date) # changing the data format to POSIXct class
...
eyes_15_2019_b.3<-eyes_15_2019_t2

eyes_15_2019_b.3<-cbind(eyes_15_2019_b.3, nestbox='mw15') 
eyes_15_2019_b.3<-cbind(eyes_15_2019_b.3, year='2019')
head(eyes_15_2019_b.3)
...
latency_eyes_15_2019<-eyes_15_2019_b.3%>%
...
latency_final_eyes_15_2019<-eyes_15_2019_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_15_2019)
glimpse(latency_final_eyes_15_2019)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_15_2019,file="totalVisits45m_mw15_2019.csv")
