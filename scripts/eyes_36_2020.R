...
setwd("~/GitHub/great_tits/data/2020 Visit counters/190520_MW36_eyes_MW22_control_MW45_eyes")# setting the working directory

eyes_36_2020<-read.delim(file= "C1252RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_36_2020)


#🧽 CLEANING DATA ---- 

eyes_36_2020_f<-eyes_36_2020%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_36_2020_f) # flituring the data


names(eyes_36_2020_f)[names(eyes_36_2020_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_36_2020_f$RFID)
uniqueIDs #lets you see the different ids in the df
# ""           "TagID_1 "   "0300030EFF" "01103FE319""01103FDB47" "01103FE413" "01103F51E9" "01103F9A3A"

eyes_36_2020_f<-eyes_36_2020_f[!grepl('TagID_1', eyes_36_2020_f$RFID),]# removing tagid one 

eyes_36_2020_f<-eyes_36_2020_f[!eyes_36_2020_f$RFID=="",]#remove blank rows
head(eyes_36_2020_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_36_2020_f$RFID)
uniqueID #only 3 different individuals
# "0300030EFF" "01103FE319" "01103FDB47" "01103FE413""01103F51E9" "01103F9A3A"

#"0300024FEF", "0300030EFF" -> is the start and stop indicator 

referencetag<-eyes_36_2020_f[eyes_36_2020_f$RFID == '0300030EFF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used
# different time written down online 


eyes_36_2020_f<-eyes_36_2020_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_36_2020_f$Date) # changing the data format to POSIXct class
...
eyes_36_2020_t<-subset(eyes_36_2020_f,Date >= as.POSIXct('2020-05-19 08:52:46', tz="UTC")) # do from last indicator

#this time is 45 minutes from the above time manually write this
eyes_36_2020_t2<-subset(eyes_36_2020_t,Date <= as.POSIXct('2020-05-19 09:41:13', tz="UTC")) # make sure 45 mins
# missing the final date 

# 📅 DATE ----
# adding the date and the nest box name to the df

eyes_36_2020_b.3<-eyes_36_2020_t2

eyes_36_2020_b.3<-cbind(eyes_36_2020_b.3, nestbox='mw36') 
eyes_36_2020_b.3<-cbind(eyes_36_2020_b.3, year='2020')
head(eyes_36_2020_b.3)
...
latency_eyes_36_2020<-eyes_36_2020_b.3%>%
...
latency_final_eyes_36_2020<-eyes_36_2020_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_36_2020)
glimpse(latency_final_eyes_36_2020)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_36_2020,file="totalVisits45m_mw36_2020.csv")
