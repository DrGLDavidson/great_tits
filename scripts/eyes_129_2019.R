...
setwd("~/GitHub/great_tits/data/2019 Visit counters/100519_MW129_eyes")# setting the working directory

eyes_129_2019<-read.delim(file= "C1256RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_129_2019)


#🧽 CLEANING DATA ---- 

eyes_129_2019_f<-eyes_129_2019%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_129_2019_f) # flituring the data


names(eyes_129_2019_f)[names(eyes_129_2019_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_129_2019_f$RFID)
uniqueIDs #lets you see the different ids in the df
#""           "TagID_1 "   "0300024FEF" "01103F4609" "0110172636"


eyes_129_2019_f<-eyes_129_2019_f[!grepl('TagID_1', eyes_129_2019_f$RFID),]# removing tagid one 

eyes_129_2019_f<-eyes_129_2019_f[!eyes_129_2019_f$RFID=="",]#remove blank rows
head(eyes_129_2019_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_129_2019_f$RFID)
uniqueID #only 3 different individuals
#"0300024FEF" "01103F4609" "0110172636"

#"0300024FEF", "0300030EFF" -> is the start and stop indicator 

referencetag<-eyes_129_2019_f[eyes_129_2019_f$RFID == '0300024FEF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used


eyes_129_2019_f<-eyes_129_2019_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_129_2019_f$Date) # changing the data format to POSIXct class
...
eyes_129_2019_t<-subset(eyes_129_2019_f,Date >= as.POSIXct('2019-05-10 09:15:16', tz="UTC")) # do from last indicator

#this time is 45 minutes from the above time manually write this
eyes_129_2019_t2<-subset(eyes_129_2019_t,Date <= as.POSIXct('2019-05-10 10:01:32', tz="UTC")) # make sure 45 mins
...
eyes_129_2019_b.3<-eyes_129_2019_t2

eyes_129_2019_b.3<-cbind(eyes_129_2019_b.3, nestbox='mw129') 
eyes_129_2019_b.3<-cbind(eyes_129_2019_b.3, year='2019')
head(eyes_129_2019_b.3)
...
latency_eyes_129_2019<-eyes_129_2019_b.3%>%
...
latency_final_eyes_129_2019<-eyes_129_2019_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_129_2019)
glimpse(latency_final_eyes_129_2019)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_129_2019,file="totalVisits45m_mw129_2019.csv")
