...
eyes_6_2019<-read.delim(file= "C1256RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_6_2019)


#🧽 CLEANING DATA ---- 

eyes_6_2019_f<-eyes_6_2019%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_6_2019_f) # flituring the data


names(eyes_6_2019_f)[names(eyes_6_2019_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_6_2019_f$RFID)
...
eyes_6_2019_f<-eyes_6_2019_f[!grepl('TagID_1', eyes_6_2019_f$RFID),]# removing tagid one 

eyes_6_2019_f<-eyes_6_2019_f[!eyes_6_2019_f$RFID=="",]#remove blank rows
head(eyes_6_2019_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_6_2019_f$RFID)
...
referencetag<-eyes_6_2019_f[eyes_6_2019_f$RFID == '0300024FEF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used


eyes_6_2019_f<-eyes_6_2019_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_6_2019_f$Date) # changing the data format to POSIXct class
...
eyes_6_2019_t<-subset(eyes_6_2019_f,Date >= as.POSIXct('2019-05-09 13:14:22', tz="UTC")) # do from last indicator

#this time is 45 minutes from the above time manually write this
eyes_6_2019_t2<-subset(eyes_6_2019_t,Date <= as.POSIXct('2019-05-09 14:01:25 ', tz="UTC")) # make sure 45 mins
...
eyes_6_2019_b.3<-eyes_6_2019_t2

eyes_6_2019_b.3<-cbind(eyes_6_2019_b.3, nestbox='mw6') 
eyes_6_2019_b.3<-cbind(eyes_6_2019_b.3, year='2019')
head(eyes_6_2019_b.3)
...
latency_eyes_6_2019<-eyes_6_2019_b.3%>%
...
latency_final_eyes_6_2019<-eyes_6_2019_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_6_2019)
glimpse(latency_final_eyes_6_2019)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_6_2019,file="totalVisits45m_mw6_2019.csv")
