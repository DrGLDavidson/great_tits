...
setwd("~/GitHub/great_tits/data/2019 Visit counters/100519_MW113_eyes_MW27_eyes")# setting the working directory

eyes_113_2019<-read.delim(file= "C1252RT.TXT", sep="\t", header=TRUE)#loading in data
names(eyes_113_2019)


#🧽 CLEANING DATA ---- 

eyes_113_2019_f<-eyes_113_2019%>%
  select(Date, Hmsec, CounterID, TagID_1)
names(eyes_113_2019_f) # flituring the data


names(eyes_113_2019_f)[names(eyes_113_2019_f) == "TagID_1"] <- "RFID" # renaming RFID 


uniqueIDs<-unique(eyes_113_2019_f$RFID)
uniqueIDs #lets you see the different ids in the df
#""           "TagID_1 "   "0300024FEF" "01103F4609" "0110172636"


eyes_113_2019_f<-eyes_113_2019_f[!grepl('TagID_1', eyes_113_2019_f$RFID),]# removing tagid one 

eyes_113_2019_f<-eyes_113_2019_f[!eyes_113_2019_f$RFID=="",]#remove blank rows
head(eyes_113_2019_f)

# 🌎 DATA EXPLORATION ----

uniqueID<- unique(eyes_113_2019_f$RFID)
uniqueID #only 3 different individuals
#"0300030EFF" "01101715AA" "01101701B5" "0419483034" "0110174FC8"
#"0300024FEF", "0300030EFF" -> is the start and stop indicator 

referencetag<-eyes_113_2019_f[eyes_113_2019_f$RFID == '0300030EFF',]#flituring out 0300030EFF
referencetag # can see when the indicator was used


eyes_113_2019_f<-eyes_113_2019_f%>%
  mutate(Date=ymd_hms(Date))
class(eyes_113_2019_f$Date) # changing the data format to POSIXct class
...
eyes_113_2019_t<-subset(eyes_113_2019_f,Date >= as.POSIXct('2019-05-10 09:08:28', tz="UTC")) # do from last indicator

#this time is 45 minutes from the above time manually write this
eyes_113_2019_t2<-subset(eyes_113_2019_t,Date <= as.POSIXct('2019-05-10 09:55:34', tz="UTC")) # make sure 45 mins
...
eyes_113_2019_b.3<-eyes_113_2019_t2

eyes_113_2019_b.3<-cbind(eyes_113_2019_b.3, nestbox='mw113') 
eyes_113_2019_b.3<-cbind(eyes_113_2019_b.3, year='2019')
head(eyes_113_2019_b.3)
...
latency_eyes_113_2019<-eyes_113_2019_b.3%>%
...
latency_final_eyes_113_2019<-eyes_113_2019_b.3%>%
  arrange(Date)%>%
  mutate(latency = Date - Date[row_number()==1])%>%
  group_by(RFID)%>%
  slice(1)%>%
...
head(latency_final_eyes_113_2019)
glimpse(latency_final_eyes_113_2019)

#💾 SAVING----
setwd("~/GitHub/great_tits/scripts")
write.csv(latency_final_eyes_113_2019,file="totalVisits45m_mw113_2019.csv")
